...
```{r}
asv <- read_tsv("/Users/lmoore/Desktop/Documents/CSU/PhD/ITS/ITS_table.txt")
metadata <- read_csv("/Users/lmoore/Desktop/Documents/CSU/PhD/16s/Grazing_Microbiome/metadata.csv") 
```
I was not able to rarefy my ITS table as I would have lost too many samples, therefore, I chose to use relative abundance to normalize my table instead. 

```{r}
# Convert ASV counts to relative abundances
asv <- asv %>%
  mutate(across(-OTU, ~ . / sum(.), .names = "{.col}")) 
```

```{r}
# Apply square root transformation to all sample columns, keeping OTU intact
asv <- asv %>%
  mutate(across(-OTU, sqrt))
...
metadata <- metadata %>%
  select(Sample, depth, depth_num, Depth_Practice, Location, Adaptive_Score, Practice, 
         Location_Depth, Biomass, Depth_Practice,
         percent_clay, percent_sand, percent_silt, pH, 
         total_c, maom_c, pom_c, total_n, maom_n, pom_n, 
         fmaoc, fpoc, c_n_ratio, mean_annual_temp, mean_annual_precip, 
         radiation, PET)
...
If you want to include an environmental fit, this is where you create a df to do that.
```{r}
# Select the environmental variables you want to add to the NMDS
envfit.data <- metadata %>%
  select(Sample, pH, total_c, maom_c, pom_c, total_n, 
         percent_clay, percent_sand, percent_silt, 
         mean_annual_temp_scaled, mean_annual_precip, 
         PET, radiation)
...
flip <- asv %>% 
 ...
 #select(1, contains("_D3_"))  %>% # if you want to filter out certain samples could be done here
  pivot_longer(2:88, names_to = "Sample", values_to = "count") %>% 
...
```{r}
envfit.data <- envfit.data %>%
  filter(Sample %in% flip$Sample)
```
...
mantel_results <- list(
  ...
  "PET" = run_mantel_test(meta$PET, data.t), 
...
#write.csv(mantel_df, file = "/Users/lmoore/Desktop/Documents/CSU/PhD/Dissertation/Grazing Microbiome/Data_Figures/Mantel/mantel_results_rarefied.csv")
```
...
#envfit.data <- read_csv("/Users/lmoore/Desktop/Documents/CSU/PhD/16s/Grazing_Microbiome/envfit_test.csv") # text file of enviornmental variables
...
```{r}
NMDS <- metaMDS(data.t, k = 3, trymax = 200)  # Increase k to 3 dimensions

```

This only works with NMDS of 3 dimensions
```{r}
stressplot(NMDS, NMDS$diss)
...
**Must use the NMDS with 2 dimensions for plotting
